...
export class BussinesFlowComponent implements OnInit {
  ...
  table1Data: Expenses[] = dugovanja; 
  table2Data: Expenses[] = uplate;
  ...
  updateSummary(){
    ...
    this.table1Data.forEach(x => {
      this.summaryTable1 += x.total_price;
    });
    
    this.table2Data.forEach(x => {
      this.summaryTable2 += x.total_price;
    });
  ...
  filterTables(selectValue:string){
    ...
    if(selectValue != "~"){
      this.table1Data = this.table1Data.filter( x => x.customer==selectValue);
      this.table2Data = this.table2Data.filter( x => x.customer == selectValue);
    }
    this.updateSummary();
  }
}
